...
RSpec.describe PurchaseHistory, type: :model do
  ...
  it "金額(price)が数値であれば有効であること" do
    history = PurchaseHistory.new(item: item, group: group, bought_at: Time.current, price: 500)
    expect(history).to be_valid
  end

  it "金額(price)が空でも有効であること（家計簿は任意入力なので）" do
    history = PurchaseHistory.new(item: item, group: group, bought_at: Time.current, price: nil)
    expect(history).to be_valid
  end

  it "金額(price)に文字列が入った場合は無効であること" do
    history = PurchaseHistory.new(item: item, group: group, bought_at: Time.current, price: "五百円")
    expect(history).to_not be_valid
  end
end